...
void segment(uint8_t n, int x, int y, int w, int h, uint16_t c) {
  /*A*/
  if (n & 0b10000000) square(x, y, w + h, h, c);
  /*B*/
  if (n & 0b01000000) square(x + w, y, h, w + h, c);
  /*C*/
  if (n & 0b00100000) square(x + w, y + w, h, w + h, c);
  /*D*/
  if (n & 0b00010000) square(x, y + 2 * w, w + h, h, c);
  /*E*/
  if (n & 0b00001000) square(x, y + w, h, w + h, c);
  /*F*/
  if (n & 0b00000100) square(x, y, h, w + h, c);
  /*G*/
  if (n & 0b00000010) square(x, y + w, w + h, h, c);
}
...
void digit(uint8_t n, int x, int y, int w, int h, uint16_t c) {
  switch (n) {
  case 0:
    segment(0b11111100, x, y, w, h, c);
    break;
  case 1:
    segment(0b01100000, x, y, w, h, c);
    break;
  case 2:
    segment(0b11011010, x, y, w, h, c);
    break;
  case 3:
    segment(0b11110010, x, y, w, h, c);
    break;
  case 4:
    segment(0b01100110, x, y, w, h, c);
    break;
  case 5:
    segment(0b10110110, x, y, w, h, c);
    break;
  case 6:
    segment(0b10111110, x, y, w, h, c);
    break;
  case 7:
    segment(0b11100000, x, y, w, h, c);
    break;
  case 8:
    segment(0b11111110, x, y, w, h, c);
    break;
  case 9:
    segment(0b11110110, x, y, w, h, c);
    break;
  }
}
...
void drawplayfield() {
  ...
  for (int x = 0; x < (int)width; x += 28) square(x, height / 2 - 3, 14, 6, W);
  //draw the scores
...
bool input() {
  ...
  if (testKey(key1, key2, KEY_RIGHT)) {
    if (px[0] < (int)width - pw) px[0] += pv;
  }
  if (testKey(key1, key2, KEY_1)) {
    if (px[1] > 0) px[1] -= pv;
  }
  if (testKey(key1, key2, KEY_3)) {
    if (px[1] < (int)width - pw) px[1] += pv;
  }
